...
def generate_boxes(masks, fallback_ratio=0.25):
    """
    Convert binary masks to bounding box prompts for SAM.

    Args:
        masks (torch.Tensor): shape (B, 1, H, W) or (B, H, W)
        fallback_ratio (float): fallback box size fraction if mask is empty

    Returns:
        torch.Tensor: shape (B, 4), each box is [x0, y0, x1, y1]
    """
    if masks.dim() == 4:
